...
namespace ImpReflection
{
	...
	{
	public:
		MapType() : Type("std::map<>", sizeof(std::map<T1, T2>), TypeCategory::Map),
		keyType{ TypeConverter<T1>::Get() }, valueType{ TypeConverter<T2>::Get() }
		{
			std::string fullName = "std::map<";
			...
			TypeManager::GetInstance()->AddTypeInfomation(
				fullName, { std::type_index(typeid(std::map<T1,T2>)),this });
		}
		...
		{
			const std::map<T1, T2>* map = reinterpret_cast<const std::map<T1, T2>*>(object);
		...
		{
			std::map<T1, T2>* map = reinterpret_cast<std::map<T1, T2>*>(object);
			map->clear(); 
