...
ImpGraphics::AnimationObject::AnimationObject(std::shared_ptr<GeometryBuffer> geometryBuffer, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture /*= nullptr*/)
{
...
ImpGraphics::AnimationObject::~AnimationObject()
{

}

void ImpGraphics::AnimationObject::Update(ImpCamera* camera)
{
	_viewTM = camera->GetViewTM();
	_projTM = camera->GetProjTM();
}

void ImpGraphics::AnimationObject::Render(ImpDevice* device, ImpRenderState* renderState, ImpRenderTarget* rendertarget)
{
	// 1st Pass
	...
	for (UINT i = 0; i < _geometryBuffer->GetSize(); i++)
	{
		...
		DirectX::XMMATRIX nodeWorld = DirectX::XMMATRIX(&_geometryBuffer->GetWorldTransform(i)._11);
		DirectX::XMMATRIX world = XMLoadFloat4x4(&_worldTM);
		DirectX::XMMATRIX worldViewProj = (nodeWorld * world) * view * proj;
...
void ImpGraphics::AnimationObject::SetTransformMatrix(DirectX::XMMATRIX transformMatrix)
{
	XMStoreFloat4x4(&_worldTM, transformMatrix);
}

std::shared_ptr<ImpGraphics::GeometryBuffer> AnimationObject::GetGeometryBuffer()
{
